...
## 37. Appendix P: Advanced Evaluation & Rating Systems

ARC Explainer utilizes multiple sophisticated ranking systems to evaluate both AI models and their specific explanations.

### 37.1 The Elo Rating System (`eloService.ts`)
**Purpose:** While standard accuracy metrics evaluate 1-shot performance, the Elo system crowdsources human evaluations to rank the *quality* and *helpfulness* of AI explanations.
**Internal Workings:**
1.  **Pair Generation (`getComparisonPair`):** The backend queries `EloRepository.ts` to find two distinct explanations (A and B) for the same puzzle that have predicted output grids.
2.  **Voting (`recordVote`):** Users vote on the UI (`A_WINS`, `B_WINS`, or `BOTH_BAD`).
3.  **Math Engine:** The system calculates the expected win probability based on current Elo ratings (defaulting to 1500). If a 1200-rated explanation beats a 1600-rated explanation, the rating shift (K-factor) is significantly larger than if the 1600-rated one won.
4.  **Leaderboards:** `/api/elo/leaderboard` returns the top-rated individual explanations, while `/api/elo/models` aggregates these scores to rank models by their average "human helpfulness" Elo.

### 37.2 The LLM Council (`LLMCouncil.tsx`)
**Purpose:** A multi-model consensus evaluation tool. Instead of relying on a single LLM, it forces multiple models to independently solve a puzzle, evaluate each other's answers, and elect a "Chairman" to synthesize a final ruling.
**Internal Workings:**
1.  **Stage 1 (Solve/Assess):** The backend hits multiple configured models in parallel. If in 'solve' mode, they generate novel predictions. If in 'assess' mode, they critique existing explanations from the DB.
2.  **Stage 2 (Rank):** The models are shown all Stage 1 responses (anonymized as Model A, Model B) and forced to rank them using `<Badge>` elements.
3.  **Stage 3 (Synthesis):** The highest-ranked model (The Chairman) takes the aggregate rankings and writes a final, definitive answer.
**Code Fact:** The UI uses a dense tabbed layout (`TabsContent value="synthesis" | "stage1" | "stage2" | "aggregate"`) to let researchers drill down into how the consensus was formed.

## 38. Appendix Q: The Saturn Visual Solver

The Saturn solver represents ARC Explainer's most advanced integration of multi-phase reasoning and Python-driven visual data.

### 38.1 Architecture (`saturnService.ts` & `saturn_wrapper.py`)
**Purpose:** Instead of forcing an LLM to solve an ARC puzzle in one shot using a massive text prompt, Saturn breaks the puzzle down into phases and utilizes actual PNG images generated by a Python subprocess.
**Internal Workings:**
1.  **Phase 1 (Initial Analysis):** Node calls `pythonBridge.runGridVisualization()` to generate base64 images of the first training pair. It prompts the LLM (`gpt-5-nano` or `grok-4-fast-reasoning`): *"Analyze the transformation from input to output."*
2.  **Phase 2 (Prediction):** Node sends the *input* of the second training pair as an image. It uses the **Responses API** (`previousResponseId`) to maintain context. Prompt: *"Predict what the output should be."*
3.  **Phase 2.5 (Refinement):** Node reveals the *actual* output of the second training pair. Prompt: *"Compare this with your prediction. Refine your understanding."*
4.  **Phase 3 (Test Application):** Node sends the final test input. Prompt: *"Generate the output grid by applying the consistent pattern."*
**Why this matters:** This mimics human cognitive processes (hypothesis generation -> testing -> refinement) and drastically improves accuracy on complex spatial tasks.

### 38.2 Streaming Saturn Data
Because Saturn relies on multiple sequential LLM calls, the `StreamingHarness` in `BaseAIService` is overridden.
**Code Fact:**
```typescript
const wrappedHarness = harness ? {
  ...harness,
  end: async (summary) => {
    // Intercept the end() call. Do not close the SSE stream yet!
    // We are bridging multiple API calls across Phase 1, 2, and 3.
  }
} : undefined;
```
The `saturnService.ts` explicitly controls when to emit `stream.status` with `phase: 'saturn_phase2_complete'` and pushes base64 images through the SSE pipeline so the React UI can render them in real-time.

## 39. Appendix R: The Hall of Fame (`HumanTradingCards.tsx`)

**Purpose:** A gamified, visually dense "Trading Card" UI to celebrate the human researchers, competition winners, and founders of the ARC-AGI ecosystem.
**Internal Workings:**
*   Fetches data via `useArcContributors()`.
*   **Data Normalization:** Automatically handles complex team structures. For example, if the DB returns `"Team NVARC (Jean-François Puget & Ivan Sorokin)"`, the frontend splits the string using regex `/\((.*?)\)/`, instantiating two distinct `HumanTradingCard` components while preserving their shared 1st Place ranking.
*   **Theming:** Implements heavy CSS gradients (`from-amber-500/10 via-amber-400/10 to-zinc-900/60`), glowing borders, and Lucide icons to denote categories (Founders, 2025 Paper Awards, Pioneers).
*   **First Visit Animation:** Uses the `useFirstVisit()` hook. If it's a user's first time loading the page, it triggers a fullscreen `<CardPackOpening />` CSS animation sequence before revealing the Hall of Fame grid.

---
*Final End of Master Feature & Technical Specification Blueprint*

## 40. Appendix S: The ARC3 & Community Game Runners

The ARC Explainer repo includes deep integrations for ARC-AGI-3 (interactive, grid-based puzzle games). Because playing a game requires maintaining state across multiple turns (unlike ARC1/2 which are 1-shot inputs), the platform utilizes dedicated Python "Runners" that operate autonomously as long-lived subprocesses.

### 40.1 ARC3 OpenRouter Runner (`arc3_openrouter_runner.py`)
**Purpose:** A specialized LangGraph-style thinking agent designed specifically to play official ARC3 games on `three.arcprize.org` using OpenRouter models (e.g., `xiaomi/mimo-v2-flash:free`).
**Internal Workings:**
1.  **State Persistence:** The runner implements an "Observation Journal" and a "Thoughts Journal". At every turn, it adds insights like "Action effect: 15 pixels changed" to its memory, dynamically appending this to the system prompt for the next turn.
2.  **Pydantic Structured Outputs:** It forces the LLM to output decisions conforming to a strict `ActionDecision` schema (requiring an `action`, `reasoning`, and optional `coordinates` for `ACTION6` clicks).
3.  **Frame Delta Analysis:** It calculates the pixel differences between the previous frame and the current frame. If an action resulted in 0 pixels changed, the runner realizes it is "stuck" and logs a thought to try a different direction, mimicking human trial-and-error learning.
4.  **API Bridging:** The Python script itself instantiates an `Arc3ApiClient`, handles opening the scorecard (`open_scorecard`), executing actions (`POST /api/cmd/ACTION1`), and closing the scorecard upon a `WIN` or `GAME_OVER`. The Node.js backend merely observes the NDJSON event stream to update the React UI.

### 40.2 Community Game Runner (`community_game_runner.py`)
**Purpose:** Allows users to play and test custom, user-submitted ARC3 games (built using the `ARCEngine` library) locally without needing an official `three.arcprize.org` API connection.
**Internal Workings:**
1.  **Dynamic Imports:** When Node.js spawns this runner, it passes a `game_path` in the JSON payload. The runner dynamically loads the user's Python file using `importlib.util.spec_from_file_location()`.
2.  **Continuous Execution Loop:** It sits in a `for line in sys.stdin:` loop. The Node backend sends JSON commands like `{"action": "ACTION1"}`. The Python script calls the ARCEngine `game.perform_action()`, parses the resulting 3D frame array, and emits the new grid state back to Node via `stdout`.
3.  **Coordinate Mapping:** Converts `{"action": "ACTION6", "coordinates": [5, 10]}` into ARCEngine's `ActionInput(id=GameAction.ACTION6, data={"x": 5, "y": 10})`.

## 41. Appendix T: Deep Dive into SnakeBench Execution

SnakeBench is the primary environment used in "Worm Arena" to test the spatial reasoning and collision-avoidance capabilities of LLMs.

### 41.1 The SnakeBench Runner (`snakebench_runner.py`)
**Purpose:** To orchestrate a head-to-head Snake match between two LLMs (e.g., GPT-5 vs. Claude 3.5) utilizing the `SnakeBench` engine.
**Internal Workings:**
1.  **Dependency Isolation:** It explicitly bypasses SnakeBench's native Supabase database connection (`os.environ.setdefault("SNAKEBENCH_DISABLE_SUPABASE", "1")`), ensuring the ARC Explainer Node.js backend remains the sole source of truth for match history and ELO tracking.
2.  **Dynamic Configuration:** The runner dynamically constructs `player_config` objects for the `LLMPlayer` module on the fly, injecting pricing models and verbosity configurations (e.g., setting `text.verbosity: "medium"` for OpenAI Responses API compatibility).
3.  **Match Output:** After `run_simulation()` completes, it maps the internal snake IDs ("0" and "1") back to the requested model names (e.g., "openai/gpt-5-nano") and resolves the exact file path where the `snake_game_<id>.json` replay file was written on disk. It returns this compact summary to Node.js.

### 41.2 Node.js Orchestration (`SnakeBenchStreamingRunner.ts` & `SnakeBenchMatchRunner.ts`)
**Purpose:** Node.js manages the lifecycle of the Python subprocess.
**Internal Workings:**
1.  **Streaming vs. Headless:** ARC Explainer maintains two distinct Node classes. `SnakeBenchMatchRunner` runs matches entirely in the background (used for batch processing). `SnakeBenchStreamingRunner` hooks into the `readline` interface of `stdout`, broadcasting `stream.chunk` events so users on `/worm-arena/live/:sessionId` can watch the snakes move frame-by-frame.
2.  **Safety Net:** Implements a strict execution timeout. If the Python process hangs (e.g., an LLM API failure upstream), Node kills the child process after a predefined threshold and marks the match as failed in the database.
